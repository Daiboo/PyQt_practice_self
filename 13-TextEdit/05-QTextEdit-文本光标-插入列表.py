...
class Window(QWidget):
    ...
    def cursor_insert1(self):
        tc = self.te.textCursor()   # 创建一个文本光标对象
        ...
        # 在当前位置插入一个新块，并使其成为给定格式的新创建列表的第一个列表项。返回创建的列表
    ...
    def cursor_insert2(self):
        ...
        tlf = QTextListFormat()
        tlf.setIndent(2)  # 设置列表的缩进为3
        tlf.setNumberPrefix("<<")   # 设置前缀（只有在列表前有数字时才生效）
        tlf.setNumberSuffix(">>")  # 设置后缀（自由在列表前有数字时才生效）
        tlf.setStyle(QTextListFormat.ListDecimal)  # 样式为小写阿拉伯数字
        tc.createList(tlf)
