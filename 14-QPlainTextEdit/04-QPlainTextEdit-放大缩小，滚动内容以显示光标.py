...
class Window(QWidget):
    ...
    def setup_ui(self):
        ...
        # ------放大缩小------------------------
        btn1.clicked.connect(lambda:pte.zoomIn(1))  # zoomIn() 中int值为正则放大，为负则缩小，绝对值越大变化越快

        # ------滚动保证光标可见--------------------------------
        """比如正在编辑一个长文本的中间段落,光标位置不变,去临时阅读以下文首的内容后,期望直接返回光标所在位置继续编辑"""
        pte.setCenterOnScroll(True)  # 一开始  可以滚动到中间位置，会在末尾增加一些空白来使得最后一行文本也能处于edit的中间

        def test_slot():
            pte.centerCursor()  # 尽可能使当前光标所在行滚动到整个edit框中间
            # pte.ensureCursorVisible()  # 确保Edit中显示光标，如果光标已经在显示范围内则不滚动，否则进行centerCursor()
