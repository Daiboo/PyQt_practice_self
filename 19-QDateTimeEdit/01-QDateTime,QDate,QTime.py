...
class Window(QWidget):
    ...
    def setup_ui(self):
        ...
        # # ----------创建------------
        # dt = QDateTime() 
        # te = QDateTime(2021,5,8,12,30)  # 创建时设置年，月，日，时，分
        dt = QDateTime.currentDateTime()  # 静态方法，获取当前时间
        dt = QDateTime.currentDateTimeUtc()  # 静态方法，获取当前的UTC时间
        print("dt",dt)

        # -------调整日期时间---------
        dt_add_year = dt.addYears(1)  # 增加一年，返回一个新的QDateTime对象
        ...
        # ------------展示时间-----------
        dte = QDateTimeEdit(dt,self)  # 方便查看起见，展示以下dt的信息
        dte.move(100,100)
        ...
        print(QDate.currentDate())  # 获取当前时间
        ...
        print("day of year",qd.dayOfYear())  # 这一年的第几天
        print("day in month",qd.daysInMonth())  # 这一个月总共有多少天
        print("days in year",qd.daysInYear())  # 这一年总共有多少天
        ...
        my_time.start()  # 开始计时
        time.sleep(1)
        print(my_time.elapsed())  # 从开始计时到执行此函数中间所用的时间，单位ms
