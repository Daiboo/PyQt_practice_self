...
le.textChanged.connect(lambda val:print("文本框内容发生改变",val))  # 改变时发生的信号，传出文本框当前内容
le.setText("用代码改变文本框内容,以测试信号")  # 会触发textChanged信号:不是用户做出的编辑，不会触发textEdited信号

# le.returnPressed.connect(lambda:print("回车键被按下了"))
# le.editingFinished.connect(lambda:print("结束编辑"))  # 失去焦点时认为是结束编辑，发射这个信号
# le.cursorPositionChanged.connect(lambda old_pos,new_pos:print(old_pos,new_pos))  #  光标的移动发射信号，传出新旧两个坐标的位置参数
