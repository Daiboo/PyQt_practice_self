...
class Window(QWidget):
    ...
    def setup_ui(self):
        te = QTextEdit("一开始就有的文字",self)  # 虽然已经写入了文字，但光标会在首行
        te.setText("第二次设置文字")   # 会把之前的文字替换掉
        self.te = te
    ...
    def 文本内容的设置(self):
        # 设置普通文本
        # self.te.setPlainText("<h3>h3标题</h3>")  # 会先清空，再以纯文本写入，光标默认置于前面
        # self.te.insertPlainText("<h4>h4标题</h4>") # 不会先清空
        # print(self.te.toPlainText())  # 以纯文本返回内容

        # 富文本
        self.te.setHtml("<h3>h3标题</h3>") 
        self.te.setHtml("<h4>h4标题</h4>")
        print(self.te.toHtml()) # 转换成html4 之后输出
