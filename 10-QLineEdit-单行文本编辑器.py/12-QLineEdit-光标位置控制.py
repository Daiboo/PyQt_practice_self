...
def cursor_move():
    ...
    # le.cursorWordBackward(True)  # 向后移动一个单词长度
    # le.cursorWordForward(True)   # 向前移动一个单词的长度
    # le.home(False)  # 移动光标到首行
    # le.end(False)   # 移动光标到尾行
    # le.setCursorPosition(int(len(le.text())/2))  # 移动光标到行中
    # print(le.cursorPosition())
    print(le.cursorPositionAt(QPoint(55,5)))  # 获取这一点坐标的字符，在字符串出现的位置
    le.setFocus()
